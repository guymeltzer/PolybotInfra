...
resource "null_resource" "argocd_direct_access" {
  ...
  provisioner "local-exec" {
    ...
    command     = <<-EOT
      ...
      # Create an improved SSH tunnel script incorporating best practices from user's script
...
TIMEOUT_SECONDS=300  # 5 minutes timeout for pod readiness
...
function cleanup() {
  status "info" "Cleaning up connections..."
  
  # Kill local SSH tunnel
  if pgrep -f "ssh.*-L $LOCAL_PORT:localhost:$LOCAL_PORT.*$CONTROL_PLANE_IP" > /dev/null; then
    status "info" "Terminating local SSH tunnel..."
    pkill -f "ssh.*-L $LOCAL_PORT:localhost:$LOCAL_PORT.*$CONTROL_PLANE_IP" > /dev/null 2>&1
  fi
  
  # Kill remote kubectl port-forward
  status "info" "Terminating remote port forwarding..."
  ssh -i "$SSH_KEY" -o ConnectTimeout=5 -o StrictHostKeyChecking=no ubuntu@$CONTROL_PLANE_IP \
    "pkill -f 'kubectl.*port-forward.*$ARGOCD_SERVICE'" > /dev/null 2>&1 || true
    
  status "success" "Tunnel closed successfully."
  exit 0
}
...
# Check prerequisites
status "info" "Checking prerequisites..."

if ! command -v ssh &> /dev/null; then
  status "error" "SSH client not found. Please install it."
  exit 1
fi

if ! command -v lsof &> /dev/null; then
  status "warning" "lsof not found. Port checking will be limited."
fi

# Check SSH key
if [ ! -f "$SSH_KEY" ]; then
  status "error" "SSH key not found at $SSH_KEY"
  exit 1
fi
...
status "info" "Checking connection to control plane ($CONTROL_PLANE_IP)..."
if ! ping -c 1 -W 2 $CONTROL_PLANE_IP > /dev/null 2>&1; then
  status "warning" "Control plane not responding to ping (may be normal if ICMP is blocked)."
fi
...
if ! ssh -i "$SSH_KEY" -o ConnectTimeout=5 -o StrictHostKeyChecking=no -o BatchMode=yes \
  ...
  status "error" "Cannot SSH to control plane. Check your SSH key and network connectivity."
  exit 1
fi

# Check and free local port if needed
status "info" "Checking local port $LOCAL_PORT..."
if lsof -ti:$LOCAL_PORT > /dev/null 2>&1; then
  status "warning" "Port $LOCAL_PORT is already in use, attempting to free it..."
  lsof -ti:$LOCAL_PORT | xargs kill -9 > /dev/null 2>&1
  sleep 1
  
  if lsof -ti:$LOCAL_PORT > /dev/null 2>&1; then
    status "error" "Could not free port $LOCAL_PORT. Please close applications using it or change the port."
    exit 1
  fi
  status "success" "Port $LOCAL_PORT freed successfully."
else
  status "success" "Local port $LOCAL_PORT is available."
fi

# Check remote port forwarding and clean up if needed
status "info" "Checking remote port forwarding..."
ssh -i "$SSH_KEY" -o ConnectTimeout=5 -o StrictHostKeyChecking=no ubuntu@$CONTROL_PLANE_IP \
  "if pgrep -f 'kubectl.*port-forward.*$ARGOCD_SERVICE' > /dev/null; then pkill -f 'kubectl.*port-forward.*$ARGOCD_SERVICE'; echo 'Killed existing port-forward'; fi" \
  > /dev/null 2>&1

# Wait for ArgoCD pod to be ready
status "info" "Checking if ArgoCD pod is ready..."
...
if [ "$READY_STATUS" != "True" ]; then
  status "warning" "ArgoCD pod not ready. Waiting up to $TIMEOUT_SECONDS seconds..."
  
  START_TIME=$(date +%s)
  while true; do
    ELAPSED_TIME=$(( $(date +%s) - START_TIME ))
    if [ $ELAPSED_TIME -ge $TIMEOUT_SECONDS ]; then
      status "warning" "Timed out waiting for ArgoCD pod to become ready."
      status "info" "Continuing anyway, but ArgoCD might not be accessible yet."
      break
    fi
    ...
    if [ "$READY_STATUS" = "True" ]; then
      status "success" "ArgoCD pod is ready!"
      break
    fi
    
    echo -ne "${YELLOW}⏳ Waiting for ArgoCD pod to be ready... (${ELAPSED_TIME}s/${TIMEOUT_SECONDS}s)${NC}\r"
    sleep 5
  done
  echo ""
else
  status "success" "ArgoCD pod is already ready!"
fi

# Setup port forwarding
status "info" "Setting up port forwarding on control plane..."
ssh -i "$SSH_KEY" -o ConnectTimeout=5 -o StrictHostKeyChecking=no ubuntu@$CONTROL_PLANE_IP \
...
if [ "$PF_CHECK" = "none" ]; then
  status "error" "Failed to set up port forwarding on control plane."
  exit 1
fi

status "success" "Port forwarding is active on control plane (PID: $PF_CHECK)."

# Get password for display
PASSWORD=$(cat /tmp/argocd-admin-password.txt 2>/dev/null || echo "Not available - check with kubectl")

# Start SSH tunnel
status "info" "Starting SSH tunnel to ArgoCD..."

echo -e "\n${BOLD}${BLUE}==========================================${NC}"
echo -e "${BOLD}${BLUE}       ArgoCD Access Information${NC}"
echo -e "${BOLD}${BLUE}==========================================${NC}"
echo -e "${BOLD}URL:${NC}      https://localhost:$LOCAL_PORT"
echo -e "${BOLD}Username:${NC} admin"
echo -e "${BOLD}Password:${NC} $PASSWORD"
echo -e "\n${YELLOW}Press Ctrl+C to close the connection${NC}"
echo -e "${BLUE}==========================================${NC}\n"
...
# Should never get here as the above command will block until Ctrl+C
# But if it does, the cleanup function will handle termination
status "info" "SSH tunnel closed."
EOF
