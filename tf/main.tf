...
resource "null_resource" "argocd_direct_access" {
  ...
  provisioner "local-exec" {
    ...
    command     = <<-EOT
      ...
      # Create an improved SSH tunnel script with better error handling and cleanup
      # Use the heredoc with 'cat' as a delimiter to avoid variable interpolation
      cat > ~/argocd-ssh-tunnel.sh << 'TUNNEL_EOF'
#!/bin/bash
...
cleanup() {
  ...
  # Kill local tunnel if running
  pkill -f "ssh.*-L ${LOCAL_PORT}:localhost:${LOCAL_PORT}.*${CONTROL_PLANE_IP}" > /dev/null 2>&1 || true
  
  # Also kill the remote port-forward
  if [ -n "$BG_PID" ]; then
    ssh -i "$SSH_KEY" -o ConnectTimeout=5 -o StrictHostKeyChecking=no ubuntu@$CONTROL_PLANE_IP "pkill -f 'kubectl.*port-forward.*${SERVICE}';" > /dev/null 2>&1 || true
  fi
...
if lsof -ti:$LOCAL_PORT > /dev/null 2>&1; then
  ...
  # Check again
  if lsof -ti:$LOCAL_PORT > /dev/null 2>&1; then
    echo "Could not free port $LOCAL_PORT. Please close applications using it or change LOCAL_PORT."
    exit 1
  fi
  
  echo "Successfully cleared port $LOCAL_PORT."
fi

# Kill any existing kubectl port-forward on the remote machine
echo "Setting up port forwarding to ArgoCD..."
ssh -i "$SSH_KEY" -o ConnectTimeout=5 -o StrictHostKeyChecking=no ubuntu@$CONTROL_PLANE_IP "pkill -f 'kubectl.*port-forward.*${SERVICE}';" > /dev/null 2>&1 || true

# Start port forwarding on the remote machine
echo "Starting port forwarding on the control plane..."
ssh -i "$SSH_KEY" -o ConnectTimeout=5 -o StrictHostKeyChecking=no ubuntu@$CONTROL_PLANE_IP "kubectl port-forward -n $NAMESPACE svc/$SERVICE ${LOCAL_PORT}:${REMOTE_PORT} --address 0.0.0.0 > /dev/null 2>&1 &" || {
  echo "Failed to establish port forwarding on the control plane."
  exit 1
}

# Wait briefly for the port forwarding to start
sleep 2

# Start SSH tunnel in the foreground
echo "Starting SSH tunnel on local machine..."
echo "ArgoCD will be available at: https://localhost:$LOCAL_PORT"
echo "Username: admin"
...
ssh -i "$SSH_KEY" -L ${LOCAL_PORT}:localhost:${LOCAL_PORT} -N -o ExitOnForwardFailure=yes -o ConnectTimeout=5 -o StrictHostKeyChecking=no ubuntu@$CONTROL_PLANE_IP

# The cleanup function will handle exit
TUNNEL_EOF

      # Now replace the CONTROL_PLANE_IP placeholder with the actual value
      sed -i.bak "s/CONTROL_PLANE_IP=\"\${CONTROL_PLANE_IP}\"/CONTROL_PLANE_IP=\"$CONTROL_PLANE_IP\"/" ~/argocd-ssh-tunnel.sh
      rm -f ~/argocd-ssh-tunnel.sh.bak

      chmod 755 ~/argocd-ssh-tunnel.sh
      
      # Create a clean instructions file without ANSI color codes
...
- Set up port forwarding on the control plane
- Create an SSH tunnel to access ArgoCD
- Properly clean up when you press Ctrl+C

=====================================================
EOF

      # Display a clean, simple message without heredoc markers or color codes
