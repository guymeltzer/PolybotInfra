...
resource "terraform_data" "kubectl_provider_config" {
  ...
  provisioner "local-exec" {
    command = "cat /tmp/admin_conf.txt | sed \"s|server:.*|server: https://$(cat /tmp/public_ip.txt):6443|\" > /tmp/modified_admin.conf"
  }

  # We'll use a direct approach instead of attempting kubectl commands during terraform
  provisioner "local-exec" {
    command = "echo 'Created kubeconfig file at /tmp/modified_admin.conf for external use'"
  }
